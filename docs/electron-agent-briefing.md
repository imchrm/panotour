...
> **`packages/electron/` не существует в репозитории** — его нужно создать с нуля.
> Все остальные пакеты (`viewer`, `tiler`, `editor`, `server`) уже реализованы и находятся
> в репо. Их не трогать — только использовать как зависимости или копировать при экспорте.
