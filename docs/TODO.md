...
- [x] **U.14** Подтверждение удаления сцены: модальное окно (в стиле U.13) с текстом
      «Delete scene "{title}"? This removes the panorama and its tiles from disk.
      This cannot be undone.», кнопки Cancel и Delete (красная);
      если на сцену ссылаются nav-хотспоты других сцен — дописать в предупреждение
      «N hotspot(s) in other scenes link here and will break»;
      срабатывает для крестика в списке сцен (другие пути удаления сцены отсутствуют)
