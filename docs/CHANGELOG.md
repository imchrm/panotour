...
## [2026-07-13] — Portable .exe: сборка проверена end-to-end

**Пакет:** electron, root

**Что сделано:**
- `electron-builder` откачен на 24.13.3: коллектор зависимостей 25.x терял
  хоистнутые транзитивные пакеты npm workspaces (`archiver-utils`) — упакованное
  приложение падало на старте
- Версия Electron закреплена точно (`33.4.11`): builder не находит хоистнутый
  модуль и отказывается работать с диапазоном `^33.0.0`
- `files` в `electron-builder.yml` переведён на glob `*.js` (в поимённом списке
  отсутствовали `log.js` и `window-state.js`); из renderer исключён `viewer/**`
  (дублировался с extraFiles)
- Корневой скрипт `npm run verify`; `media:copy` покрыт headless-проверкой
- Проверено на Windows: portable .exe собирается, запускается, открывает проект

**Сломано / известные проблемы:** распаковка winCodeSign требует прав на
симлинки (Developer Mode или запуск от администратора); Defender может
залочить запись exe — папку проекта в исключения.
